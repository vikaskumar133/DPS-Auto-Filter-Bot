...
async def pm_text(bot, message):
    ...
    await message.reply_text(
         ...
         reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ", url=f"https://t.me/+--xsrx3VUEBlZmY1")]])
    )
...
async def next_page(bot, query):
    ...
    if settings['button']:
		...
		# for file in files:
            ...
            btn = [
			    ...
			    text=f"[{get_size(file.file_size)}] {file.file_name.split()}", callback_data=f'files#{file.file_id}'
                        #text=f"<b>\n📁 <a href='https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}'>[{get_size(file.file_size)}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}\n</a></b>", callback_data=f'files#{file.file_id}'
                    ),
                ]
                for file in files
            ]
    ...
    btn.insert(0,
        [   
	    InlineKeyboardButton(text="⚡ʜᴏᴡ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ⚡", url='https://t.me/howtopenlinksa/12')
        ]   
    )
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            InlineKeyboardButton('★ You Can Add Our Bot To Your Groups ★', url=f'http://t.me/{temp.U_NAME}?startgroup=true')
            ],[
            InlineKeyboardButton('✪ Movie Search Group ', url=f"https://t.me/+--xsrx3VUEBlZmY1")
            ],[
            InlineKeyboardButton('✼ Movie Updates ', url=f"https://t.me/dps_movies"),
            InlineKeyboardButton('✴ Main Channel ', url='https://t.me/dps_backup')
            ],[
        ...
        if 2 < len(message.text) < 100:
            ...
            if not files:
                ...
                l = await message.reply_text(text=f"△ 𝙷𝚎𝚢 𝚜𝚘𝚗𝚊 `{message.from_user.first_name}` 😎,\n\nʏᴏᴜʀ ʀᴇQᴜᴇꜱᴛ ʜᴀꜱ ʙᴇᴇɴ ꜱᴇɴᴛ ᴛᴏ ᴏᴜʀ **ᴀᴅᴍɪɴ'ꜱ ᴅᴀꜱʜʙᴏᴀʀᴅ** !\nᴘʟᴇᴀꜱᴇ ᴋᴇᴇᴘ ꜱᴏᴍᴇ ᴘᴀᴛɪᴇɴᴄᴇ !\nᴛʜᴇʏ ᴡɪʟʟ ᴜᴘʟᴏᴀᴅ ɪᴛ ᴀꜱ ꜱᴏᴏɴ ᴀꜱ ᴘᴏꜱꜱɪʙʟᴇ.\n\n➟ 📝𝘾𝙤𝙣𝙩𝙚𝙣𝙩 𝙣𝙖𝙢𝙚 : `{search}`\n➟ 👮𝙍𝙚𝙦𝙪𝙚𝙨𝙩𝙚𝙙 𝘽𝙮 : `{message.from_user.first_name}`",
                                                                                                       reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("━ •  You Can Add Our Bot To Your Group  • ━", url=f'http://t.me/{temp.U_NAME}?startgroup=true')],[InlineKeyboardButton(" Movie Updates ", url="https://t.me/dps_movies"), InlineKeyboardButton(" Main Channel ", url="https://t.me/dps_backup")]]))
